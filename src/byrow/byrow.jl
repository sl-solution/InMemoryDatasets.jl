...
byrow(ds::AbstractDataset, ::typeof(sum), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true) = byrow(ds, sum, [col]; by = by, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(prod), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true) = byrow(ds, prod, [col]; by = by, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(count), col::ColumnIndex = names(ds, Union{Missing, Number}); by = x->true, threads = true) = byrow(ds, count, [col], by = by, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(mean), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true) = byrow(ds, mean, [col]; by = by, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(maximum), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true) = byrow(ds, maximum, [col]; by = by, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(minimum), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true) = byrow(ds, minimum, [col]; by = by, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(var), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, dof = true, threads = true) = byrow(ds, var, [col]; by = by, dof = dof, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(std), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, dof = true, threads = true) = byrow(ds, std, [col]; by = by, dof = dof, threads = threads)
...
byrow(ds::AbstractDataset, ::typeof(nunique), col::ColumnIndex = names(ds, Union{Missing, Number}); by = identity, count_missing = true) = byrow(ds, nunique, [col]; by = by, count_missing = count_missing)

byrow(ds::AbstractDataset, ::typeof(cumsum), cols::MultiColumnIndex = names(ds, Union{Missing, Number})) = row_cumsum(ds, cols)
byrow(ds::AbstractDataset, ::typeof(cumsum), col::ColumnIndex = names(ds, Union{Missing, Number})) = byrow(ds, cumsum, [col])

byrow(ds::AbstractDataset, ::typeof(cumprod!), cols::MultiColumnIndex = names(ds, Union{Missing, Number})) = row_cumprod!(ds, cols)
byrow(ds::AbstractDataset, ::typeof(cumprod!), col::ColumnIndex = names(ds, Union{Missing, Number})) = byrow(ds, cumprod!, [col])
...
byrow(ds::AbstractDataset, ::typeof(sort), col::ColumnIndex = names(ds, Union{Missing, Number}); threads = true, kwargs...) = byrow(ds, sort, [col]; threads = threads, kwargs...)
