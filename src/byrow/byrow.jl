...
byrow(ds::AbstractDataset, ::typeof(sum), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true && nrow(ds)>1000) = threads ? hp_row_sum(ds, by, cols) : row_sum(ds, by, cols)
byrow(ds::AbstractDataset, ::typeof(sum), col::ColumnIndex; by = identity, threads = true && nrow(ds)>1000) = byrow(ds, sum, [col]; by = by, threads = threads)

byrow(ds::AbstractDataset, ::typeof(prod), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true && nrow(ds)>1000) = threads ? hp_row_prod(ds, by, cols) : row_prod(ds, by, cols)
byrow(ds::AbstractDataset, ::typeof(prod), col::ColumnIndex; by = identity, threads = true && nrow(ds)>1000) = byrow(ds, prod, [col]; by = by, threads = threads)


byrow(ds::AbstractDataset, ::typeof(count), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = isequal(true), threads = true && nrow(ds)>1000) = threads ? hp_row_count(ds, by, cols) : row_count(ds, by, cols)
byrow(ds::AbstractDataset, ::typeof(count), col::ColumnIndex; by = isequal(true), threads = true && nrow(ds)>1000) = byrow(ds, count, [col], by = by, threads = threads)

# byrow(ds::AbstractDataset, ::typeof(anymissing), cols::MultiColumnIndex = names(ds, Union{Missing, Number})) = row_anymissing(ds, cols)

function byrow(ds::AbstractDataset, ::typeof(any), cols::MultiColumnIndex = :; by = x->isequal(true, x), threads = true && nrow(ds)>1000)
...
byrow(ds::AbstractDataset, ::typeof(any), col::ColumnIndex; by = x->isequal(true, x), threads = true && nrow(ds)>1000) = byrow(ds, any, [col]; by = by, threads = threads)

function byrow(ds::AbstractDataset, ::typeof(all), cols::MultiColumnIndex = :; by = x->isequal(true, x), threads = true && nrow(ds)>1000)
	colsidx = index(ds)[cols]
	if by isa AbstractVector
		bys = map(_bool, by)
...
byrow(ds::AbstractDataset, ::typeof(maximum), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true && nrow(ds)>1000) = threads ? hp_row_maximum(ds, by, cols) : row_maximum(ds, by, cols)
byrow(ds::AbstractDataset, ::typeof(maximum), col::ColumnIndex; by = identity, threads = true && nrow(ds)>1000) = byrow(ds, maximum, [col]; by = by, threads = threads)

byrow(ds::AbstractDataset, ::typeof(minimum), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = identity, threads = true && nrow(ds)>1000) = threads ? hp_row_minimum(ds, by, cols) : row_minimum(ds, by, cols)
byrow(ds::AbstractDataset, ::typeof(minimum), col::ColumnIndex; by = identity, threads = true && nrow(ds)>1000) = byrow(ds, minimum, [col]; by = by, threads = threads)

byrow(ds::AbstractDataset, ::typeof(var), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = identity, dof = true, threads = true && nrow(ds)>1000) = threads ? hp_row_var(ds, by, cols; dof = dof) : row_var(ds, by, cols; dof = dof)
byrow(ds::AbstractDataset, ::typeof(var), col::ColumnIndex; by = identity, dof = true, threads = true && nrow(ds)>1000) = byrow(ds, var, [col]; by = by, dof = dof, threads = threads)

byrow(ds::AbstractDataset, ::typeof(std), cols::MultiColumnIndex = names(ds, Union{Missing, Number}); by = identity, dof = true, threads = true && nrow(ds)>1000) = threads ? hp_row_std(ds, by, cols; dof = dof) : row_std(ds, by, cols; dof = dof)
byrow(ds::AbstractDataset, ::typeof(std), col::ColumnIndex; by = identity, dof = true, threads = true && nrow(ds)>1000) = byrow(ds, std, [col]; by = by, dof = dof, threads = threads)
...
function byrow(ds::AbstractDataset, f::Function, col::ColumnIndex; threads = true && nrow(ds)>1000)
	if threads
