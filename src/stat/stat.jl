...
Base.argmax(x::AbstractArray{Union{T,Missing},1}; by = identity) where T <: Union{INTEGERS, FLOATS, TimeType, AbstractString} = stat_findmax(by, x)[2]
Base.argmin(x::AbstractArray{Union{T,Missing},1}; by = identity) where T <: Union{INTEGERS, FLOATS, TimeType, AbstractString} = stat_findmin(by, x)[2]
