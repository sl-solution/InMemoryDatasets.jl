...
# ########### AsTable is here just temporary to figure out how it works
struct AsTable
    cols
end
...
const TRANSFORMATION_COMMON_RULES =
    ...
    `InMemoryDatasets.jl` are explained and compared.

    All these operations are supported both for
    `AbstractDataset` (when split and combine steps are skipped) and
    `GroupedDataset`. Technically, `AbstractDataset` is just considered as being
    ...
    keyword arguments (described below) are not supported and a data set is always
    returned, as there are no split and combine steps in this case.

    In order to perform operations by groups you first need to create a `GroupedDataset`
    object from your data set using the `groupby` function that takes two arguments:
    (1) a data set to be grouped, and (2) a set of columns to group by.
    ...
    * `combine`: does not put restrictions on number of rows returned, the order of rows
      is specified by the order of groups in `GroupedDataset`; it is typically used
      to compute summary statistics by group;
    * `select`: return a data set with the number and order of rows exactly the same
      as the source data set, including only new calculated columns;
      `select!` is an in-place version of `select`;
    * `transform`: return a data set with the number and order of rows exactly the same
      as the source data set, including all columns from the source and new calculated columns;
    ...
    each subset of the `Dataset`. This specification can be of the following forms:
    ...
    8. a function which will be called with a `SubDataset` corresponding to each group;
       ...
       (in which case `SubDataset` avoids excessive compilation)

    All functions have two types of signatures. One of them takes a `GroupedDataset`
    ...
    is passed as the first argument and a `GroupedDataset` as the second argument
    (similar to `map`).
    ...
    1. If both `cols` and `target_cols` are omitted (so only a `function` is passed),
       then returning a data set, a matrix, a `NamedTuple`, or a `DatasetRow` will
    ...
    2. If `target_cols` is a `Symbol` or a string then the function is assumed to return
       a single column. In this case returning a data set, a matrix, a `NamedTuple`,
       or a `DatasetRow` raises an error.
    3. If `target_cols` is a vector of `Symbol`s or strings or `AsTable` it is assumed
       that `function` returns multiple columns.
       If `function` returns one of `AbstractDataset`, `NamedTuple`, `DatasetRow`,
    ...
    `select`/`select!` and `transform`/`transform!` always return a `Dataset`
    with the same number and order of rows as the source (even if `GroupedDataset`
    had its groups reordered).

    For `combine`, rows in the returned object appear in the order of groups in the
    `GroupedDataset`. The functions can return an arbitrary number of rows for
    each group, but the kind of returned object and the number and names of columns
    must be the same for all groups, except when a `Dataset()` or `NamedTuple()`
    ...
    names in target data set are accepted (e.g. `select!(df, [:a], :, r"a")`
    is allowed) and only the first occurrence is used. In particular a syntax to
    move column `:col` to the first position in the data set is
    ...
    including the same column several times in the data set via renaming or
    ...
    copies columns to avoid storing aliased columns in the target data set.

    If `df` is a `SubDataset` and `copycols=true` then a `Dataset` is
    returned and the same copying rules apply as for a `Dataset` input: this
    means in particular that selected columns will be copied. If
    `copycols=false`, a `SubDataset` is returned without copying columns.

    If a `GroupedDataset` is passed, a separate task is spawned for each
...
_transformation_helper(df::AbstractDataset, col_idx::Nothing, (fun,)::Ref{Any}) =
    fun(df)
_transformation_helper(df::AbstractDataset, col_idx::Int, (fun,)::Ref{Any}) =
    fun(df[!, col_idx])

_empty_astable_helper(fun, len) = [fun(NamedTuple()) for _ in 1:len]

function _transformation_helper(df::AbstractDataset, col_idx::AsTable, (fun,)::Ref{Any})
...
function _transformation_helper(df::AbstractDataset, col_idx::AbstractVector{Int}, (fun,)::Ref{Any})
...
function _insert_row_multicolumn(newdf::Dataset, df::AbstractDataset,
                                 allow_resizing_newdf::Ref{Bool}, colnames::AbstractVector{Symbol},
                                 @nospecialize(res::Union{NamedTuple, DatasetRow}))
    if ncol(newdf) == 0
...
function _fix_existing_columns_for_vector(newdf::Dataset, df::AbstractDataset,
    ...
    if !allow_resizing_newdf[] && ncol(newdf) == 0 && lr != nrow(df)
        throw(ArgumentError("length $(lr) of vector returned from " *
                            ...
                            "$(nrow(df)) of the source data set."))
    end
    allow_resizing_newdf[] = false
end

function _add_col_check_copy(newdf::Dataset, df::AbstractDataset,
...
function _add_multicol_res(res::AbstractDataset, newdf::Dataset, df::AbstractDataset,
...
function _add_multicol_res(res::AbstractMatrix, newdf::Dataset, df::AbstractDataset,
                           colnames::AbstractVector{Symbol},
                           allow_resizing_newdf::Ref{Bool}, wfun::Ref{Any},
                           col_idx::Union{Nothing, Int, AbstractVector{Int}, AsTable},
                           copycols::Bool, newname::Union{Nothing, Type{AsTable}, AbstractVector{Symbol}})
...
function _add_multicol_res(@nospecialize(res::NamedTuple{<:Any, <:Tuple{Vararg{AbstractVector}}}),
                           newdf::Dataset, df::AbstractDataset,
                           colnames::AbstractVector{Symbol},
                           allow_resizing_newdf::Ref{Bool}, wfun::Ref{Any},
                           col_idx::Union{Nothing, Int, AbstractVector{Int}, AsTable},
                           copycols::Bool, newname::Union{Nothing, Type{AsTable}, AbstractVector{Symbol}})
...
function _add_multicol_res(@nospecialize(res::NamedTuple), newdf::Dataset, df::AbstractDataset,
                           colnames::AbstractVector{Symbol},
                           allow_resizing_newdf::Ref{Bool}, wfun::Ref{Any},
                           col_idx::Union{Nothing, Int, AbstractVector{Int}, AsTable},
                           copycols::Bool, newname::Union{Nothing, Type{AsTable}, AbstractVector{Symbol}})
...
function _add_multicol_res(res::DatasetRow, newdf::Dataset, df::AbstractDataset,
                           colnames::AbstractVector{Symbol},
                           allow_resizing_newdf::Ref{Bool}, wfun::Ref{Any},
                           col_idx::Union{Nothing, Int, AbstractVector{Int}, AsTable},
                           copycols::Bool, newname::Union{Nothing, Type{AsTable}, AbstractVector{Symbol}})
...
function select_transform!((nc,)::Ref{Any}, df::AbstractDataset, newdf::Dataset,
    ...
    if newname === AsTable || newname isa AbstractVector{Symbol}
        if res isa AbstractVector && !isempty(res)
            ...
            res = newres
        elseif !(res isa Union{AbstractDataset, NamedTuple, DatasetRow, AbstractMatrix})
            res = Tables.columntable(res)
        end
    end

    if res isa Union{AbstractDataset, NamedTuple, DatasetRow, AbstractMatrix}
...
"""
    select!(df::Dataset, args...; renamecols::Bool=true)
    select!(args::Base.Callable, df::Dataset; renamecols::Bool=true)
    select!(gd::GroupedDataset{Dataset}, args...; ungroup::Bool=true, renamecols::Bool=true)
    select!(f::Base.Callable, gd::GroupedDataset; ungroup::Bool=true, renamecols::Bool=true)
...
parent. If there are independent `GroupedDataset` objects constructed using
the same parent data set they might get corrupt.
...
- `ungroup::Bool=true` : whether the return value of the operation on `gd` should be a data
  set or a `GroupedDataset`.

See [`select`](@ref) for examples.
"""
select!(df::Dataset, @nospecialize(args...); renamecols::Bool=true) =
    _replace_columns!(df, select(df, args..., copycols=false, renamecols=renamecols))

function select!(@nospecialize(arg::Base.Callable), df::AbstractDataset; renamecols::Bool=true)
    if arg isa Colon
...
"""
    transform!(df::Dataset, args...; renamecols::Bool=true)
    transform!(args::Callable, df::Dataset; renamecols::Bool=true)
    transform!(gd::GroupedDataset{Dataset}, args...; ungroup::Bool=true, renamecols::Bool=true)
    transform!(f::Base.Callable, gd::GroupedDataset; ungroup::Bool=true, renamecols::Bool=true)
...
See [`select`](@ref) for examples.
"""
function transform!(df::Dataset, @nospecialize(args...); renamecols::Bool=true)
    idx = index(df)
...
function transform!(@nospecialize(arg::Base.Callable), df::AbstractDataset; renamecols::Bool=true)
    if arg isa Colon
        throw(ArgumentError("First argument must be a transformation if the second argument is a data set"))
    end
...
"""
    select(df::AbstractDataset, args...; copycols::Bool=true, renamecols::Bool=true)
    select(args::Callable, df::Dataset; renamecols::Bool=true)
    select(gd::GroupedDataset, args...; copycols::Bool=true, keepkeys::Bool=true,
           ungroup::Bool=true, renamecols::Bool=true)
    select(f::Base.Callable, gd::GroupedDataset; copycols::Bool=true,
           keepkeys::Bool=true, ungroup::Bool=true, renamecols::Bool=true)

Create a new data set that contains columns from `df` or `gd` specified by
...
# Keyword arguments
- `copycols::Bool=true` : whether columns of the source data set should be copied if
...
julia> df = Dataset(a=1:3, b=4:6, c=7:9)
3×3 Dataset
 Row │ a      b      c
     │ Int64  Int64  Int64
─────┼─────────────────────
...
julia> df = Dataset(a = [1, 1, 1, 2, 2, 1, 1, 2],
...
GroupedDataset with 2 groups based on key: a
...
"""
select(df::AbstractDataset, @nospecialize(args...); copycols::Bool=true, renamecols::Bool=true) =
...
"""
    transform(df::AbstractDataset, args...; copycols::Bool=true, renamecols::Bool=true)
    transform(f::Callable, df::Dataset; renamecols::Bool=true)
    transform(gd::GroupedDataset, args...; copycols::Bool=true,
              keepkeys::Bool=true, ungroup::Bool=true, renamecols::Bool=true)
    transform(f::Base.Callable, gd::GroupedDataset; copycols::Bool=true,
              keepkeys::Bool=true, ungroup::Bool=true, renamecols::Bool=true)

Create a new data set that contains columns from `df` or `gd` plus columns
...
Note that when the first argument is a `GroupedDataset`, `keepkeys=false`
is needed to be able to return a different value for the grouping column:

```jldoctest
julia> gdf = groupby(Dataset(x=1:2), :x)
GroupedDataset with 2 groups based on key: x
...
ERROR: ArgumentError: column :x in returned data set is not equal to grouping key :x
```

See [`select`](@ref) for more examples.
"""
function transform(df::AbstractDataset, @nospecialize(args...); copycols::Bool=true, renamecols::Bool=true)
    idx = index(df)
    # when using the copy function the copy of source data set
...
function transform(@nospecialize(arg::Base.Callable), df::AbstractDataset; renamecols::Bool=true)
    if arg isa Colon
        throw(ArgumentError("First argument to must be a transformation if the second argument is a data set"))
    end
    return transform(df, arg)
end

"""
    combine(df::AbstractDataset, args...; renamecols::Bool=true)
    combine(f::Callable, df::AbstractDataset; renamecols::Bool=true)
    combine(gd::GroupedDataset, args...;
            keepkeys::Bool=true, ungroup::Bool=true, renamecols::Bool=true)
    combine(f::Base.Callable, gd::GroupedDataset;
            keepkeys::Bool=true, ungroup::Bool=true, renamecols::Bool=true)
...
GroupedDataset with 4 groups based on key: a
...
julia> combine(gd) do sdf # dropping group when Dataset() is returned
          sdf.c[1] != 1 ? sdf : Dataset()
       end
6×3 Dataset
 Row │ a      b      c
     │ Int64  Int64  Int64
...
function _manipulate(ds::AbstractDataset, normalized_cs::Vector{Any}, copycols::Bool, keeprows::Bool)
    ...
    #   in to the target data set once and only once the first time the target column
            ...
            if ind isa ColumnIndex
                ...
                if newind isa Pair
                    throw(ArgumentError("transforming and renaming columns of a " *
                                        "SubDataset is not allowed when `copycols=false`"))
                end
                push!(newinds, newind)
            end
        end
...
function manipulate(dfv::SubDataset, args::MultiColumnIndex;
...
manipulate(df::Dataset, c::ColumnIndex; copycols::Bool, keeprows::Bool,
           renamecols::Bool) =
