...
@testset "Basic usage" begin
    ...
    dss = sort(ds, 1)
    @test dss == sort(ds, 1, threads = false)
    @test issorted(dss.x1)
    dss = sort(ds, 1, rev = true)
    @test dss == sort(ds, 1, rev = true, threads = false)
    ...
    @test dss == sort(ds, 1, rev = true, mapformats = false, threads = false)
    @test issorted(dss.x1, rev = true)
    ...
    sort!(ds2, 1)
    ds3 = sort(ds2, 1:2)
    @test ds3 == sort(ds2, 1:2, threads = false)
    @test ds3 == sort(ds, :)
    ds4 = sort(ds, 1:2, rev = true)
    @test ds4 == sort(ds, 1:2, rev = true, threads = false)
    ...
    @test sort(ds, 1:4, threads = false) == ds2
    ...
    @test sort(ds, 1:5, threads = false) == ds2
    ...
    @test issorted(sort(ds, 5, rev = true, threads = false).col5, rev = true)
    ...
    @test sortperm(d, :, threads = false) == sortperm(dv1)

    @test sortperm(d[:, [:dv3, :dv1]], :) == sortperm(dv3)
    @test sortperm(d[:, [:dv3, :dv1]], :, threads = false) == sortperm(dv3)

    @test sort(d, :dv1)[!, :dv3] == sort(d, "dv1")[!, "dv3"] == sortperm(dv1)
    @test sort(d, :dv1, threads = false)[!, :dv3] == sort(d, "dv1", threads = false)[!, "dv3"] == sortperm(dv1)

    @test sort(d, :dv2)[!, :dv3] == sortperm(dv1)
    @test sort(d, :dv2, threads = false)[!, :dv3] == sortperm(dv1)

    @test sort(d, :cv1)[!, :dv3] == sortperm(dv1)
    @test sort(d, [:dv1, :cv1])[!, :dv3] == sortperm(dv1)
    @test sort(d, [:dv1, :cv1], threads = false)[!, :dv3] == sortperm(dv1)

    @test sort(d, [:dv1, :dv3])[!, :dv3] == sortperm(dv1)
    @test sort(d, [:dv1, :dv3], threads = false)[!, :dv3] == sortperm(dv1)
    ...
    ds = Dataset(x = x)
    ds_s = sort(ds, :x)
    @test ds_s == sort(ds, :x, threads = false)
    ...
    for i in 1:20
        @test sort(ds, :, threads = false) == sort(ds_pa, :, threads = false) == ds2
    end
    x1 = -rand(1:1000, 5000)
    x2 = -rand(1:100, 5000)
    ...
    @test sortperm(dsl, :, threads = false)==sortperm(dsr, :)

    @test sortperm(dsl, :, rev=true)==sortperm(dsr, :, rev=true)
    @test sortperm(dsl, :, rev=true)==sortperm(dsr, :, rev=true, threads = false)


    ds = Dataset(rand(-100:1000000, 10^7, 3), :auto)
    sort!(ds, :)
    @test issorted(ds, :)
    @test issorted(ds, :, threads = false)
...
@testset "_find_starts_of_groups" begin
    ...
    @test r2 == ranges[1:r3]
    r1, r2, r3 = IMD._find_starts_of_groups(ds, colsidx, T, threads = false)
    @test r1 == colsidx
    @test r3 == last_valid_index
    @test r2 == ranges[1:r3]
    ...
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    r1, r2, r3 = IMD._find_starts_of_groups(gb, colsidx, T, threads = false)
    @test r1 == colsidx
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    ...
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    r1, r2, r3 = IMD._find_starts_of_groups(ds, colsidx, T, mapformats = false, threads = false) # Do not use formatted values.
    @test r1 == colsidx
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    ...
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    r1, r2, r3 = IMD._find_starts_of_groups(ds, colsidx, T, threads = false) # Use formatted values.
    @test r1 == colsidx
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    ...
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    r1, r2, r3 = IMD._find_starts_of_groups(dss, colsidx, T, mapformats = false, threads = false) # Do not use formatted values.
    @test r1 == colsidx
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    ...
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    r1, r2, r3 = IMD._find_starts_of_groups(dss, colsidx, T, threads = false) # Use formatted values.
    @test r1 == colsidx
    @test r3 == last_valid_index
    @test r2[1:r3] == ranges
    ...
    ra1, ra2, ra3 = IMD._find_starts_of_groups(ds, colsidx, T, threads = false)
    rb1, rb2, rb3 = _get_starts(ds, colsidx, T)
    @test ra1 == rb1
    @test ra3 == rb3
    @test ra2[1:ra3] == rb2[1:rb3]
...
@testset "bigInt test set" begin
    ...
    @test sortperm(ds, 1, threads = false) == [3,1,4,2,5]
    @test sortperm(ds, 1, rev = true, threads = false) == [5,2,1,4,3]
    @test sortperm(ds, 1:2, threads = false) == [3, 1, 4, 2, 5]
    @test sortperm(ds, 1:2, rev = [false, true], threads = false) == [3,4,1,2,5]
    ds[2,1]=missing
    @test sortperm(ds, 1) == [3,1,4,5,2]
    @test sortperm(ds, 1, rev = true) == [2,5,1,4,3]
    @test sortperm(ds, 1, threads = false) == [3,1,4,5,2]
    @test sortperm(ds, 1, rev = true, threads = false) == [2,5,1,4,3]
    ...
    @test sortperm(ds, 1, threads = false) == sortperm(x)
    @test sortperm(ds, 1, rev=true, threads = false) == sortperm(x, rev = true)
    ...
    @test sortperm(ds, 1, threads = false) == sortperm(x, by = isodd)
    @test sortperm(ds, 1, rev = true, threads = false) == sortperm(x, by = isodd, rev = true)
    ...
    @test sort(ds,1, threads = false) == ds[[4,2,1,3],:]
    @test sort(ds,1, rev = true, threads = false) == ds[[3,1,2,4],:]
    setformat!(ds, 1=>isodd)
    ...
    @test sort(ds,1, threads = false) == ds[[2,3,1,4],:]
    @test sort(ds,1, rev=true, threads = false) == ds[[1,4,2,3],:]
    @test sort(ds, 1:2, threads = false) == ds[[2,3,1,4], :]
    @test sort(ds, 1:2, rev = [false, true], threads = false) == ds[[3,2,4,1], :]
end
...
@testset "sort views" begin
    ...
    @test sort(ds, 1, threads = false) == sort(ds[!, [1,2]], 1, threads = false)
    @test sort(ds, 1, threads = false) == sort(ds[!, [2,1]], 2, threads = false)[:, [2,1]]
    @test sortperm(sds, 2, threads = false) == [1,3,4,5,8, 7, 2,6,9]
    @test sortperm(sds, [2,1], threads = false) == [1,3,4,5,8,7, 9, 2,6]
    if !Base.Sys.iswindows()
        ...
        for i = 1:100
            ds = Dataset(rand(1:10, 1000, 3), :auto)
            @test sort(ds, :, threads = false) == sort(ds[!, 1:3], :, threads = false)
            ds = Dataset(rand(1:1000000, 1000, 3), :auto)
            @test sort(ds, :) == sort(ds[!, 1:3], :, threads = false)
            ds = Dataset(rand(1000, 2), :auto)
            @test sort(ds, :) == sort(ds[!, 1:2], :, threads = false)
        end
    end
end
...
@testset "issorted/issorted!" begin
    ...
    @test !issorted(d, :cv1, threads = false)
    @test issorted(d, :dv3, threads = false)
    @test !issorted(d, :dv1, threads = false)
    ...
    @test issorted(d, :cv1, threads = false)
    @test issorted(d, :dv1, threads = false)
    @test !issorted(d, :dv2, threads = false)

    ds = Dataset(x = [0xfffffffffffffff3, 0xfffffffffffffff2, 0xfffffffffffffff4, 0xfffffffffffffff1], y = [1,1,2,2])
    @test issorted(ds[[4,2,1,3],:],1)
    @test issorted(view(ds, [4,2,1,3], :), 1)
    @test issorted(ds[[3,1,2,4],:],1, rev = true)
    @test issorted(ds[[4,2,1,3],:],1, threads = false)
    @test issorted(view(ds, [4,2,1,3], :), 1, threads = false)
    @test issorted(ds[[3,1,2,4],:],1, rev = true, threads = false)
    setformat!(ds, 1=>isodd)
    ...
    @test issorted(ds[[2,3,1,4],:],1, threads = false)
    @test issorted(view(ds, [2,3,1,4], :), 1, threads = false)
    @test issorted(ds[[1,4,2,3],:],1, rev=true, threads = false)
    @test issorted(ds[[2,3,1,4], :], 1:2, threads = false)
    @test issorted(view(ds, [2,3,1,4], :), 1:2, threads = false)
    @test issorted(ds[[3,2,4,1], :], 1:2, rev = [false, true], threads = false)
    @test issorted(view(ds, [3,2,4,1], :), 1:2, rev = [false, true], threads = false)
    ...
    @test issorted(sort(ds, 1),1, threads = false)
    @test issorted(sort(ds, 1, rev = true), 1, rev=true, threads = false)
    setformat!(ds, 1=>isodd)
    @test issorted(sort(ds, 1),1)
    @test issorted(sort(ds, 1, rev = true), 1, rev = true)
    ...
    @test !issorted(ds, 1, threads = false)
    @test issorted(ds[[3,1,4,2,5], 1:1], 1, threads = false)
    @test issorted(view(ds, [5,2,1,4,3], [2,1]), 2, rev = true, threads = false)
    @test issorted(ds[[3, 1, 4, 2, 5], :], 1:2, threads = false)
    @test issorted(ds[[3,4,1,2,5],:], 1:2, rev = [false, true], threads = false)
    ...
    @test !issorted(ds, 1, threads = false)
    @test issorted(ds[[3,1,4,5,2], :], 1, threads = false)
    @test issorted(view(ds, [2,5,1,4,3], :), 1, rev = true, threads = false)
    if !Base.Sys.iswindows()
        for i in 1:100
        ...
        for i in 1:100
            ...
            for j in 1:10
                sort!(ds, 1:2:j, threads = false)
                issorted!(ds, 1:2:j, threads = false)
                @test IMD._sortedcols(ds) == collect(1:2:j)
                @test issorted(ds, 1:2:j, threads = false)

                setformat!(ds, 1:10=>sign)
                sort!(ds, 1:2:j, rev = true, threads = false)
                issorted!(ds, 1:2:j, rev = true, threads = false)
                @test IMD._sortedcols(ds) == collect(1:2:j)
                @test issorted(ds, 1:2:j, rev = true, threads = false)
            end
        end
    end
